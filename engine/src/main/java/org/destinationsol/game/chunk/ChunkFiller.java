...
public class ChunkFiller {
    ...
    private FarShip buildSpaceEnemy(SolGame game, Vector2 position, RemoveController remover, ShipConfig enemyConf) {
        ...
        SolMath.fromAl(speed, SolMath.rnd(180), SolMath.rnd(0, ENEMY_MAX_SPD));
        float rotationSpeed = SolMath.rnd(ENEMY_MAX_ROT_SPD);
    ...
    private void fillAsteroids(SolGame game, RemoveController remover, boolean forBelt, Vector2 chunkCenter) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            Vector2 speed = new Vector2();
            SolMath.fromAl(speed, SolMath.rnd(180), MAX_A_SPD);
    ...
    private void fillFarJunk(SolGame game, Vector2 chunkCenter, RemoveController remover, DrawableLevel drawableLevel,
        ...
        for (int i = 0; i < count; i++) {
            ...
            TextureAtlas.AtlasRegion tex = new TextureAtlas.AtlasRegion(SolMath.elemRnd(conf.farJunkTextures));
            ...
            float rotationSpeed = SolMath.rnd(FAR_JUNK_MAX_ROT_SPD);
            ...
            RectSprite s = new RectSprite(tex, sz, 0, 0, junkPos, drawableLevel, SolMath.rnd(180), rotationSpeed, SolColor.DDG, false);
            drawables.add(s);
        }
    ...
    private void fillJunk(SolGame game, RemoveController remover, SpaceEnvConfig conf, Vector2 chunkCenter) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            // Apply a random rotation speed
            float rotationSpeed = SolMath.rnd(JUNK_MAX_ROT_SPD);

            // Create the resulting sprite and add it to the list as the only element
            RectSprite s = new RectSprite(tex, sz, 0, 0, new Vector2(), DrawableLevel.DECO, SolMath.rnd(180), rotationSpeed, SolColor.LG, false);
            ArrayList<Drawable> drawables = new ArrayList<>();
            drawables.add(s);

            // Create a FarDrawable instance for this piece of junk and only allow it to be drawn when it's not hidden by a planet
            Vector2 speed = new Vector2();
            SolMath.fromAl(speed, SolMath.rnd(180), SolMath.rnd(JUNK_MAX_SPD_LEN));
    ...
    private Vector2 getRndPos(Vector2 chunkCenter) {
        ...
        position.x += SolMath.rnd(Const.CHUNK_SIZE / 2);
        position.y += SolMath.rnd(Const.CHUNK_SIZE / 2);
        return position;
    }
