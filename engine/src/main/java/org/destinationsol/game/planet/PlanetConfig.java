...
public class PlanetConfig {
    ...
    static String[] getDifficultyLevels(JSONObject rootNode, String moduleName) {
        JSONArray difficultyLevels = rootNode.getJSONArray("difficultyLevels");
        String[] arr = new String[difficultyLevels.length()];
        for(int i = 0; i < difficultyLevels.length(); i++) {
            arr[i] = difficultyLevels.getString(i);
        }
        return arr;
    }
}
