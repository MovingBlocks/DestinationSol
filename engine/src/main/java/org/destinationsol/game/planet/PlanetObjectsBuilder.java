...
public class PlanetObjectsBuilder {
    ...
    private FarPlanetSprites createCloud(Planet planet, List<TextureAtlas.AtlasRegion> cloudTextures) {
        float distancePercentage = SolMath.rnd(0, 1);
        ...
        float sizePercentage = SolMath.rnd(.2f, 1);
        ...
        float rotationSpeed = SolMath.rnd(.1f, 1) * SolMath.arcToAngle(MAX_CLOUD_LINEAR_SPD, distance);
    ...
    private void addDeco(SolGame game, float groundHeight, Vector2 planetPos,
        ...
        for (int i = 0; i < decorationCount; i++) {
            ...
            TextureAtlas.AtlasRegion decorationTexture = new TextureAtlas.AtlasRegion(SolMath.elemRnd(decoConfig.texs));
            if (decoConfig.allowFlip && SolMath.test(.5f)) {
    ...
    private FarShip buildOrbitEnemy(SolGame game, Planet planet, float heightPercentage, ShipConfig shipConfig, float detectionDistance) {
        ...
        boolean clockwise = SolMath.test(.5f);
        if (!clockwise) {
            speedLen *= -1;
        }
