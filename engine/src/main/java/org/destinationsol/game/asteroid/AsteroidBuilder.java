...
public class AsteroidBuilder {
    ...
    public Asteroid buildNew(SolGame game, Vector2 position, Vector2 speed, float size, RemoveController removeController) {
        float rotationSpeed = SolMath.rnd(MAX_A_ROT_SPD);
        return build(game, position, SolMath.elemRnd(textures), size, SolMath.rnd(180), rotationSpeed, speed, removeController);
    }

    // doesn't consume position
    public FarAsteroid buildNewFar(Vector2 position, Vector2 speed, float size, RemoveController removeController) {
        float rotationSpeed = SolMath.rnd(MAX_A_ROT_SPD);
        return new FarAsteroid(SolMath.elemRnd(textures), new Vector2(position), SolMath.rnd(180), removeController, size, new Vector2(speed), rotationSpeed);
    }
