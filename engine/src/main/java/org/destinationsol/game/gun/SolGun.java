...
public class SolGun {
    ...
    private void shoot(Vector2 gunVelocity, SolGame game, float gunAngle, Vector2 muzzlePos, Faction faction, SolObject creator) {
        ...
        for (int i = 0; i < cc.projectilesPerShot; i++) {
            ...
            Projectile proj = new Projectile(game, bulletAngle, muzzlePos, baseVelocity, faction, cc.projConfig, multiple);
    ...
    public void update(ItemContainer itemContainer, SolGame game, float gunAngle, SolObject creator, boolean shouldShoot, Faction faction, SolShip ship) {
        ...
        if (shot) {
            ...
            shoot(gunVelocity, game, gunAngle, muzzlePos, faction, creator);
        } else {
