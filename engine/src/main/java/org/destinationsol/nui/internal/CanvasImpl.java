...
import org.destinationsol.nui.events.NUIMouseDoubleClickEvent;
import org.destinationsol.nui.events.NUIMouseDragEvent;
import org.destinationsol.nui.events.NUIMouseOverEvent;
...
import org.destinationsol.nui.widgets.UITooltip;
...
public class CanvasImpl implements CanvasControl, PropertyChangeListener {
    ...
    private Vector2 lastClickPosition = new Vector2();
    ...
    public void preRender() {
        ...
        Vector2 size = renderer.getTargetSize();
        size.x = (int) (size.x / uiScale);
        size.y = (int) (size.y / uiScale);

        state = new CanvasState(null, Rectangle.createFromMinAndSize(0, 0, size.x, size.y));
        renderer.preRender();
    ...
    @Override
    public void processMousePosition(Vector2 position) {
        if (clickedRegion != null) {
            Vector2 relPos = new Vector2(position);
    ...
    @Override
    public boolean processMouseClick(MouseInput button, Vector2 pos) {
        ...
        for (InteractionRegion next : mouseOverRegions) {
            if (next.region.contains(pos)) {
                Vector2 relPos = new Vector2(pos);
                relPos.sub(next.offset);
    ...
    private NUIMouseClickEvent createClickEvent(MouseInput button, Vector2 relPos) {
        return new NUIMouseClickEvent(mouse, keyboard, relPos, button);
    }

    private NUIMouseDoubleClickEvent createDoubleClickEvent(MouseInput button, Vector2 relPos) {
        return new NUIMouseDoubleClickEvent(mouse, keyboard, relPos, button);
    }

    @Override
    public boolean processMouseRelease(MouseInput button, Vector2 pos) {
        if (clickedRegion != null) {
            Vector2 relPos = new Vector2(pos);
    ...
    @Override
    public boolean processMouseWheel(int wheelTurns, Vector2 pos) {
        for (InteractionRegion next : mouseOverRegions) {
            if (next.region.contains(pos)) {
                Vector2 relPos = new Vector2(pos);
    ...
    @Override
    public SubRegion subRegion(Rectangle region, boolean crop) {
        return new SubRegionImpl(region, crop);
    }

    @Override
    public SubRegion subRegionFBO(ResourceUrn uri, BaseVector2 size) {
    ...
    @Override
    public Vector2 size() {
        return new Vector2(state.drawRegion.width(), state.drawRegion.height());
    }

    @Override
    public Rectangle getRegion() {
        return Rectangle.createFromMinAndSize(0, 0, state.drawRegion.width(), state.drawRegion.height());
    }
    ...
    @Override
    public Vector2 calculatePreferredSize(UIWidget widget) {
        return calculateRestrictedSize(widget, new Vector2(LARGE_INT, LARGE_INT));
    }

    @Override
    public Vector2 calculateRestrictedSize(UIWidget widget, Vector2 sizeRestrictions) {
        if (widget == null) {
        ...
        Rectangle region = applyStyleToSize(Rectangle.createFromMinAndSize(Vector2.zero(), sizeRestrictions), elementStyle);
        try (SubRegion ignored = subRegionForWidget(widget, region, false)) {
            Vector2 preferredSize = widget.getPreferredContentSize(this, elementStyle.getMargin().shrink(sizeRestrictions));
    ...
    @Override
    public Vector2 calculateMaximumSize(UIWidget widget) {
        if (widget == null) {
    ...
    @Override
    public void drawWidget(UIWidget element, Rectangle region) {
        ...
        try (SubRegion ignored = subRegionForWidget(element, regionArea, false)) {
            if (element.isSkinAppliedByCanvas()) {
                drawBackground();
                try (SubRegion withMargin = subRegionForWidget(element, newStyle.getMargin().shrink(Rectangle.createFromMinAndSize(Vector2.zero(), regionArea.size())), false)) {
    ...
    private SubRegion subRegionForWidget(UIWidget widget, Rectangle region, boolean crop) {
    ...
    @Override
    public void drawText(String text, Rectangle region) {
    ...
    @Override
    public void drawTexture(TextureRegion texture, Rectangle region) {
        drawTextureRaw(texture, region, getCurrentStyle().getTextureScaleMode());
    }

    @Override
    public void drawTexture(TextureRegion texture, Rectangle region, Color color) {
        drawTextureRaw(texture, region, color, getCurrentStyle().getTextureScaleMode());
    }

    @Override
    public void drawBackground() {
        Rectangle region = applyStyleToSize(getRegion());
        drawBackground(region);
    }

    private Rectangle applyStyleToSize(Rectangle region) {
        return applyStyleToSize(region, getCurrentStyle());
    }

    private Rectangle applyStyleToSize(Rectangle region, UIStyle style, Vector2 maxSize) {
        if (region.isEmpty()) {
            return region;
        }
        Vector2 size = applyStyleToSize(region.size(), style);
        ...
        return Rectangle.createFromMinAndSize(minX, minY, size.x, size.y);
    }

    private Vector2 applyStyleToSize(Vector2 size, UIStyle style) {
    ...
    private Rectangle applyStyleToSize(Rectangle region, UIStyle style) {
        if (region.isEmpty()) {
            return region;
        }
        Vector2 size = applyStyleToSize(region.size(), style);
    ...
    @Override
    public void drawBackground(Rectangle region) {
        if (region.isEmpty()) {
            return;
        }
        UIStyle style = getCurrentStyle();
    ...
    @Override
    public void drawTextRaw(String text, Font font, Color color, Rectangle region) {
        drawTextRawShadowed(text, font, color, Color.TRANSPARENT, region);
    }

    @Override
    public void drawTextRaw(String text, Font font, Color color, Rectangle region, HorizontalAlign hAlign, VerticalAlign vAlign) {
        drawTextRawShadowed(text, font, color, Color.TRANSPARENT, region, hAlign, vAlign);
    }

    @Override
    public void drawTextRaw(String text, Font font, Color color, boolean underlined, Rectangle region, HorizontalAlign hAlign, VerticalAlign vAlign) {
    ...
    @Override
    public void drawTextRawShadowed(String text, Font font, Color color, Color shadowColor, Rectangle region) {
        drawTextRawShadowed(text, font, color, shadowColor, region, HorizontalAlign.LEFT, VerticalAlign.TOP);
    }

    @Override
    public void drawTextRawShadowed(String text, Font font, Color color, Color shadowColor, Rectangle region, HorizontalAlign hAlign, VerticalAlign vAlign) {
        drawTextRawShadowed(text, font, color, shadowColor, false, region, hAlign, vAlign);
    }

    @Override
    public void drawTextRawShadowed(String text, Font font, Color color, Color shadowColor, boolean underline, Rectangle region, HorizontalAlign hAlign, VerticalAlign vAlign) {
        Rectangle absoluteRegion = relativeToAbsolute(region);
        Rectangle cropRegion = absoluteRegion.intersect(state.cropRegion);
    ...
    @Override
    public void drawTextureRaw(TextureRegion texture, Rectangle region, ScaleMode mode) {
        drawTextureRaw(texture, region, mode, 0f, 0f, 1f, 1f);
    }

    @Override
    public void drawTextureRaw(TextureRegion texture, Rectangle region, Color color, ScaleMode mode) {
        drawTextureRaw(texture, region, color, mode, 0f, 0f, 1f, 1f);
    }

    @Override
    public void drawTextureRaw(TextureRegion texture, Rectangle region, ScaleMode mode, int ux, int uy, int uw, int uh) {
    ...
    @Override
    public void drawTextureRaw(TextureRegion texture, Rectangle region, ScaleMode mode, float ux, float uy, float uw, float uh) {
        drawTextureRaw(texture, region, Color.WHITE, mode, ux, uy, uw, uh);
    }

    @Override
    public void drawTextureRaw(TextureRegion texture, Rectangle region, Color color, ScaleMode mode, float ux, float uy, float uw, float uh) {
    ...
    @Override
    public void drawTextureRawBordered(TextureRegion texture, Rectangle region, Border border, boolean tile) {
        drawTextureRawBordered(texture, region, border, tile, 0f, 0f, 1f, 1f);
    }

    @Override
    public void drawTextureRawBordered(TextureRegion texture, Rectangle region, Border border, boolean tile, int ux, int uy, int uw, int uh) {
    ...
    @Override
    public void drawTextureRawBordered(TextureRegion texture, Rectangle region, Border border, boolean tile, float ux, float uy, float uw, float uh) {
        if (!state.cropRegion.overlaps(relativeToAbsolute(region))) {
            return;
        }
        Rectangle absoluteRegion = relativeToAbsolute(region);
    ...
    @Override
    public void drawMaterial(Material material, Rectangle region) {
        if (material.isRenderable()) {
            Rectangle drawRegion = relativeToAbsolute(region);
    ...
    @Override
    public void drawMesh(Mesh mesh, Material material, Rectangle region, Quat4f rotation, Vector3f offset, float scale) {
        if (material == null) {
    ...
    @Override
    public void drawMesh(Mesh mesh, Texture texture, Rectangle region, Quat4f rotation, Vector3f offset, float scale) {
    ...
    @Override
    public void addInteractionRegion(InteractionListener listener, String tooltip, Rectangle region) {
    ...
    @Override
    public void addInteractionRegion(InteractionListener listener, Rectangle region) {
    ...
    @Override
    public void addInteractionRegion(InteractionListener listener, UIWidget tooltip, Rectangle region) {
        Vector2 offset = state.drawRegion.min();
        Rectangle finalRegion = state.cropRegion.intersect(relativeToAbsolute(region));
    ...
    @Override
    public void drawFilledRectangle(Rectangle region, Color color) {
        drawTextureRaw(whiteTexture, region, color, ScaleMode.STRETCH);

    }

    private Rectangle relativeToAbsolute(Rectangle region) {
    ...
    private static class CanvasState {
        ...
        CanvasState(CanvasState previous, Rectangle drawRegion) {
            this(previous, drawRegion, (previous != null) ? previous.cropRegion : drawRegion);
        }

        CanvasState(CanvasState previous, Rectangle drawRegion, Rectangle cropRegion) {
            if (previous != null) {
        ...
        public Rectangle getRelativeRegion() {
            return Rectangle.createFromMinAndSize(0, 0, drawRegion.width(), drawRegion.height());
        }
    }
    ...
    private class SubRegionImpl implements SubRegion {
        ...
        SubRegionImpl(Rectangle region, boolean crop) {
            previousState = state;
            ...
            Rectangle subRegion = Rectangle.createFromMinAndMax(left, top, right, bottom);
            if (crop) {
                Rectangle cropRegion = subRegion.intersect(state.cropRegion);
                if (cropRegion.isEmpty()) {
    ...
    private final class SubRegionFBOImpl implements SubRegion {
        ...
        private SubRegionFBOImpl(ResourceUrn uri, BaseVector2 size) {
            previousState = state;

            fbo = renderer.getFBO(uri, size);
            state = new CanvasState(state, Rectangle.createFromMinAndSize(new Vector2(), size));
            fbo.bindFrame();
        }
    ...
    private static class InteractionRegion {
        ...
        InteractionRegion(Rectangle region, Vector2 offset, InteractionListener listener, UIWidget element, UIWidget tooltipOverride) {
    ...
    private final class DrawTextureOperation implements DrawOperation {
        ...
        private Rectangle absoluteRegion;
        ...
        private DrawTextureOperation(TextureRegion texture, Color color, ScaleMode mode, Rectangle absoluteRegion,
                                     Rectangle cropRegion, float ux, float uy, float uw, float uh, float alpha) {
            this.color = color;
            this.mode = mode;
    ...
    private final class DrawBorderedTextureOperation implements DrawOperation {
        ...
        private DrawBorderedTextureOperation(TextureRegion texture, Rectangle absoluteRegion, Border border, boolean tile,
                                             Rectangle cropRegion, float ux, float uy, float uw, float uh, float alpha) {
            this.texture = texture;
            this.tile = tile;
            this.absoluteRegion = absoluteRegion;
    ...
    private final class DrawTextOperation implements DrawOperation {
        ...
        private final Rectangle absoluteRegion;
        ...
        private DrawTextOperation(String text, Font font, HorizontalAlign hAlign, VerticalAlign vAlign, Rectangle absoluteRegion, Rectangle cropRegion,
    ...
    private final class DrawInteractionRegionOperation implements DrawOperation {
        ...
        DrawInteractionRegionOperation(Rectangle region, Vector2 offset, InteractionListener listener, UIWidget currentElement, UIWidget tooltipOverride) {
            this.region = region;
            this.listener = listener;
            this.offset = offset;
