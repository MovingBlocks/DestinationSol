...
public class LwjglCanvasRenderer implements CanvasRenderer, PropertyChangeListener {
    ...
    private Rectangle requestedCropRegion;
    private Rectangle currentTextureCropRegion;
    ...
    public void preRender() {
        ...
        requestedCropRegion = Rectangle.createFromMinAndSize(0, 0, Display.getWidth(), Display.getHeight());
    ...
    @Override
    public void drawMesh(Mesh mesh, Material material, Rectangle drawRegion, Rectangle cropRegion, Quat4f rotation, Vector3f offset, float scale, float alpha) {
    ...
    @Override
    public void drawMaterialAt(Material material, Rectangle drawRegion) {
        glPushMatrix();
    ...
    @Override
    public void crop(Rectangle cropRegion) {
        requestedCropRegion = cropRegion;
    }

    @Override
    public FrameBufferObject getFBO(ResourceUrn urn, BaseVector2 size) {
    ...
    @Override
    public void drawTexture(TextureRegion texture, Color color, ScaleMode mode, Rectangle absoluteRegion,
    ...
    @Override
    public void drawText(String text, Font font, HorizontalAlign hAlign, VerticalAlign vAlign, Rectangle absoluteRegion,
        ...
        Vector2 offset = new Vector2(absoluteRegion.minX(), absoluteRegion.minY());
    ...
    @Override
    public void drawTextureBordered(TextureRegion texture, Rectangle region, Border border, boolean tile, float ux, float uy, float uw, float uh, float alpha) {
        if (!texture.getTexture().isLoaded()) {
            return;
        }
        ...
        Vector2 textureSize = new Vector2(TeraMath.ceilToInt(texture.getWidth() * uw), TeraMath.ceilToInt(texture.getHeight() * uh));
        ...
        if (mesh == null || mesh.isDisposed()) {
            ...
            if (border.getTop() != 0) {
                ...
                if (tile) {
                    addTiles(builder, Rectangle.createFromMinAndSize(border.getLeft(), 0, centerHoriz, border.getTop()), Rect2f.createFromMinAndMax(left, 0, right, top),
                            new Vector2(textureSize.x - border.getTotalWidth(), border.getTop()),
            ...
            if (border.getLeft() != 0) {
                if (tile) {
                    addTiles(builder, Rectangle.createFromMinAndSize(0, border.getTop(), border.getLeft(), centerVert), Rect2f.createFromMinAndMax(0, top, left, bottom),
                            new Vector2(border.getLeft(), textureSize.y - border.getTotalHeight()),
            ...
            if (tile) {
                addTiles(builder, Rectangle.createFromMinAndSize(border.getLeft(), border.getTop(), centerHoriz, centerVert),
                        Rect2f.createFromMinAndMax(left, top, right, bottom),
                        new Vector2(textureSize.x - border.getTotalWidth(), textureSize.y - border.getTotalHeight()),
            ...
            if (border.getRight() != 0) {
                if (tile) {
                    addTiles(builder, Rectangle.createFromMinAndSize(region.width() - border.getRight(), border.getTop(), border.getRight(), centerVert),
                            Rect2f.createFromMinAndMax(right, top, 1, bottom),
                            new Vector2(border.getRight(), textureSize.y - border.getTotalHeight()),
            ...
            if (border.getBottom() != 0) {
                ...
                if (tile) {
                    addTiles(builder, Rectangle.createFromMinAndSize(border.getLeft(), region.height() - border.getBottom(), centerHoriz, border.getBottom()),
                            Rect2f.createFromMinAndMax(left, bottom, right, 1),
                            new Vector2(textureSize.x - border.getTotalWidth(), border.getBottom()),
    ...
    private void addTiles(MeshBuilder builder, Rectangle drawRegion, Rect2f subDrawRegion, Vector2 textureSize, Rect2f subTextureRegion) {
    ...
    private static class TextureCacheKey {
        ...
        TextureCacheKey(Vector2 textureSize, Vector2 areaSize) {
            this.textureSize = new Vector2(textureSize);
            this.areaSize = new Vector2(areaSize);
            this.border = Border.ZERO;
            this.tiled = true;
        }

        TextureCacheKey(Vector2 textureSize, Vector2 areaSize, Border border, boolean tiled) {
            this.textureSize = new Vector2(textureSize);
            this.areaSize = new Vector2(areaSize);
            this.border = border;
            this.tiled = tiled;
