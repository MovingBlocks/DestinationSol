...
public final class Line {
    ...
    public static LineCoordinates getLineCoordinates(int startX, int startY, int endX, int endY, Rectangle baseRegion, Rectangle cropRegion) {
        Rectangle region = Rectangle.createFromMinAndMax(Math.min(startX, endX), Math.min(startY, endY),
            Math.max(startX, endX), Math.max(startY, endY));
        Rectangle absoluteRegion = relativeToAbsolute(region, baseRegion);
        Rectangle finalRegion = cropRegion.intersect(absoluteRegion);
        ...
        if (!finalRegion.isEmpty()) {
            ...
            return new LineCoordinates(new Vector2(sx, sy), new Vector2(ex, ey));
        } else {
            return null;
        }
    }

    public static Rectangle relativeToAbsolute(Rectangle region, Rectangle baseRegion) {
        return Rectangle.createFromMinAndSize(region.minX() + baseRegion.minX(), region.minY() + baseRegion.minY(), region.width(), region.height());
    }
    ...
    public static class LineCoordinates {
        ...
        public LineCoordinates(Vector2 start, Vector2 end) {
            this.start = start;
            this.end = end;
        }
