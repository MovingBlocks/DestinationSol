 ...
 */
package org.destinationsol.nui.itemRendering;
    ...
    public void draw(T value, Canvas canvas) {
        ...
        if (texture != null) {
            if (marginTop + texture.getHeight() + marginBottom > canvas.size().y) {
                ...
                canvas.drawTexture(texture, Rectangle.createFromMinAndSize(marginLeft, marginTop, texture.getWidth(), iconHeight));
            } else {
                // Icon fits within the canvas - vertically centering it
                int iconVerticalPosition = (canvas.size().y - texture.getHeight()) / 2;
                canvas.drawTexture(texture, Rectangle.createFromMinAndSize(marginLeft, iconVerticalPosition, texture.getWidth(), texture.getHeight()));
            }
        }
        ...
        Rectangle textRegion = Rectangle.createFromMinAndSize(iconWidth, 0, canvas.getRegion().width() - iconWidth, canvas.getRegion().height());
        canvas.drawText(text, textRegion);
    }

    @Override
    public Vector2 getPreferredSize(T value, Canvas canvas) {
