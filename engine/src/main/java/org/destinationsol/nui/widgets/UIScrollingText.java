...
public class UIScrollingText extends CoreWidget {
    ...
    public void onDraw(Canvas canvas) {
        ...
        for (Entry<String, Integer> entry : textY.entrySet()) {
            ...
            if (y >= -offsetTop && y <= canvas.size().y - offsetBottom + font.getHeight(entry.getKey())) {
                ...
                Rectangle coords = Rectangle.createFromMinAndSize(w - font.getWidth(line) / 2, y, font.getWidth(line), font.getHeight(line));
     ...
     * @param areaHint A {@link Vector2} representing the available space for this {@code UIScrollingText}
     * @return A {@code Vector2} representing the preferred content size of this {@code UIScrollingText}
     */
    @Override
