...
import org.destinationsol.nui.widgets.browser.data.ParagraphData;
import org.destinationsol.nui.widgets.browser.data.basic.flow.ContainerRenderSpace;
import org.destinationsol.nui.widgets.browser.ui.style.DefaultDocumentRenderStyle;
import org.destinationsol.nui.widgets.browser.ui.style.DocumentRenderStyle;
import org.destinationsol.nui.widgets.browser.ui.style.FallbackDocumentRenderStyle;
import org.destinationsol.nui.widgets.browser.ui.style.FallbackParagraphRenderStyle;
...
public final class DocumentRenderer {
    ...
    public static Vector2 getDocumentPreferredSize(DocumentData documentData, Font defaultFont, Color defaultColor, int availableWidth) {
        ...
        return new Vector2(documentWidth, preferredHeight + documentVerticalMargins);
    }
    ...
    public static int getParagraphsPreferredHeight(ParagraphRenderStyle baseRenderStyle, Collection<ParagraphData> paragraphs,
        ...
        for (ParagraphData paragraphData : paragraphs) {
            ...
            if (floatStyle == ParagraphRenderStyle.FloatStyle.LEFT
                ...
                if (floatStyle == ParagraphRenderStyle.FloatStyle.LEFT) {
                    Rectangle position = containerRenderSpace.addLeftFloat(yShift, paragraphMinWidth, height);
                    yShift = position.minY();
                } else {
                    Rectangle position = containerRenderSpace.addRightFloat(yShift, paragraphMinWidth, height);
                    yShift = position.minY();
                }
            } else {
    ...
    public static void drawDocumentInRegion(DocumentData documentData, Canvas canvas, Font defaultFont, Color defaultColor,
                                            Vector2 size, ParagraphRenderable.HyperlinkRegister register) {
        DefaultDocumentRenderStyle defaultDocumentRenderStyle = new DefaultDocumentRenderStyle(defaultFont, defaultColor);

        DocumentRenderStyle documentRenderStyle = getDocumentRenderStyle(defaultDocumentRenderStyle, documentData);

        int documentWidth = (int) size.x;
    ...
    public static int renderParagraph(Canvas canvas, ParagraphRenderable.HyperlinkRegister register, ParagraphRenderStyle baseRenderStyle,
        ...
        if (floatStyle == ParagraphRenderStyle.FloatStyle.LEFT
            ...
            Rectangle paragraphBorderRegion =
                    Rectangle.createFromMinAndMax(position.minX() + paragraphRenderStyle.getParagraphMarginLeft().getValue(containerWidth),
            ...
            Vector2 paragraphStart = new Vector2(position.minX(),
                    position.minY()
            ...
            if (paragraphBackground != null) {
                ...
                while (borderAdvance < borderHeight) {
                    ...
                    Rectangle backgroundRegion =
                            Rectangle.createFromMinAndSize(
            ...
            paragraphContents.renderContents(canvas, new Vector2(xShift, yShift), containerRenderSpace, leftIndent + leftParagraphIndent,
