...
import org.destinationsol.nui.input.Keyboard.KeyId;
...
public class UIText extends CoreWidget {
    ...
    protected void drawAll(Canvas canvas, int multilineWidth) {
        ...
        try (SubRegion ignored = canvas.subRegion(canvas.getRegion(), true);
             SubRegion ignored2 = canvas.subRegion(Rectangle.createFromMinAndSize(-offset, 0, widthForDraw + 1, Integer.MAX_VALUE), false)) {
        ...
        for (int lineIndex = 0; lineIndex < rawLinesAfterCursor.size() && currentChar <= end; ++lineIndex) {
            ...
            for (int innerLineIndex = 0; innerLineIndex < innerLines.size() && currentChar <= end; ++innerLineIndex) {
                ...
                if (!selectionString.isEmpty()) {
                    ...
                    Vector2 selectionTopLeft = new Vector2(offsetX, (lineOffset) * font.getLineHeight());
                    Rectangle region = Rectangle.createFromMinAndSize(selectionTopLeft.x, selectionTopLeft.y, selectionWidth, font.getLineHeight());
    ...
    protected void drawCursor(Canvas canvas) {
        if (blinkCounter < BLINK_RATE) {
            ...
            Rectangle region = Rectangle.createFromMinAndSize(lastLineWidth, (lines.size() - 1) * font.getLineHeight(),
    ...
    public Vector2 getPreferredContentSize(Canvas canvas, Vector2 areaHint) {
        ...
        if (isMultiline()) {
            ...
            return new Vector2(font.getWidth(getText()), font.getLineHeight());
        }
    }
    ...
    public Vector2 getMaxContentSize(Canvas canvas) {
        ...
        if (isMultiline()) {
            ...
            return new Vector2(Integer.MAX_VALUE, font.getLineHeight());
        }
    }
     ...
     */
    protected void moveCursor(Vector2 pos, boolean selecting, KeyboardDevice keyboard) {
        if (lastFont != null) {
            pos.x += offset;
