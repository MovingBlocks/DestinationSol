...
import org.destinationsol.nui.widgets.treeView.Tree;
import org.destinationsol.nui.widgets.treeView.TreeKeyEventListener;
import org.destinationsol.nui.widgets.treeView.TreeModel;
import org.destinationsol.nui.widgets.treeView.TreeMouseClickListener;
import org.destinationsol.nui.widgets.treeView.TreeViewState;
...
public class UITreeView<T> extends CoreWidget {
    ...
    public void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < model.get().getNodeCount(); i++) {
            ...
            Rectangle nodeRegion = Rectangle.createFromMinAndSize((node.getDepth() + 1) * levelIndent.get(),
                currentHeight,
            ...
            if (!node.isLeaf()) {
                ...
                Rectangle buttonRegion = Rectangle.createFromMinAndSize(node.getDepth() * levelIndent.get(),
                    currentHeight,
                    levelIndent.get(),
                    nodeHeight);
    ...
    public Vector2 getPreferredContentSize(Canvas canvas, Vector2 sizeHint) {
        ...
        for (int i = 0; i < model.get().getNodeCount(); i++) {
            Tree<T> node = model.get().getNode(i);
            Vector2 preferredSize = canvas.getCurrentStyle().getMargin()
    ...
    private void drawButton(Canvas canvas, Rectangle buttonRegion, ExpandButtonInteractionListener listener) {
        canvas.drawBackground(buttonRegion);
        canvas.addInteractionRegion(listener, buttonRegion);
    }

    private void drawNode(Canvas canvas, Rectangle nodeRegion, Tree<T> node, TreeViewListenerSet listenerSet) {
        ...
        canvas.addInteractionRegion(listenerSet.getTopListener(), itemRenderer.getTooltip(node.getValue()),
            Rectangle.createFromMinAndSize(nodeRegion.minX(), nodeRegion.minY(),
        ...
        canvas.addInteractionRegion(listenerSet.getCenterListener(), itemRenderer.getTooltip(node.getValue()),
            Rectangle.createFromMinAndSize(nodeRegion.minX(), nodeRegion.minY() + nodeRegion.height() / 3,
                nodeRegion.width(), nodeRegion.height() / 3));
        ...
        canvas.addInteractionRegion(listenerSet.getBottomListener(), itemRenderer.getTooltip(node.getValue()),
            Rectangle.createFromMinAndSize(nodeRegion.minX(), nodeRegion.minY() + 2 * nodeRegion.height() / 3,
                nodeRegion.width(), heightOffset + nodeRegion.height() / 3));
    }

    private void drawDragHint(Canvas canvas, Rectangle nodeRegion) {
