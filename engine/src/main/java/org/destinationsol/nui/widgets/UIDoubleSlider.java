...
public class UIDoubleSlider extends CoreWidget {
    ...
    private void drawTicker(Canvas canvas, String part, Binding<Float> value, InteractionListener tickerListener, boolean rightTicker) {
        ...
        Rectangle tickerRegion = Rectangle.createFromMinAndSize(drawLocation, 0, tickerWidth, canvas.size().y);
    ...
    public Vector2 getPreferredContentSize(Canvas canvas, Vector2 areaHint) {
        ...
        Vector2 left = getTickerPreferredContentSize(canvas, TICKER_LEFT_PART);
        Vector2 right = getTickerPreferredContentSize(canvas, TICKER_RIGHT_PART);
    ...
    private Vector2 getTickerPreferredContentSize(Canvas canvas, String part) {
        Vector2 result = new Vector2();
