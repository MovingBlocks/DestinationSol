...
public class UIDropdownScrollable<T> extends UIDropdown<T> {
    ...
    public void onDraw(Canvas canvas) {
        ...
        if (!isEnabled()) {
            ...
            Rectangle frame = Rectangle.createFromMinAndSize(0, canvas.size().y, canvas.size().x, height);
     ...
     * @param itemHeight Height per menu item.
     */
    private void createScrollbarItems(Canvas canvas, Rectangle frame, Font font, Border itemMargin, int height, int itemHeight) {
        // Scrollable Area
        Rectangle scrollableArea = Rectangle.createFromMinAndSize(0, canvas.size().y, canvas.size().x, height - itemMargin.getBottom());

        // Scrollbar Measurement
        int scrollbarWidth = canvas.calculateRestrictedSize(verticalBar, new Vector2(canvas.size().x, canvas.size().y)).x;
        ...
        // Draw Scrollbar
        Rectangle scrollbarRegion = Rectangle.createFromMinAndSize(scrollbarXPos, scrollbarYPos, scrollbarWidth, scrollbarHeight);
        ...
        for (int i = 0; i < optionListeners.size(); ++i) {
            readItemMouseOver(canvas, i);
            Rectangle itemRegion = Rectangle.createFromMinAndSize(0, itemHeight * i - verticalBar.getValue(), availableWidth, itemHeight);
     ...
     */
    private void drawItem(Canvas canvas, Border itemMargin, int i, Rectangle itemRegion) {
