...
import static org.destinationsol.nui.UIWidget.ACTIVE_MODE;
...
public class UIList<T> extends ActivatableWidget {
    ...
    public void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < list.get().size(); ++i) {
            ...
            Vector2 preferredSize = margin.grow(itemRenderer.getPreferredSize(item, canvas));

            Rectangle itemRegion = Rectangle.createFromMinAndSize(0, (int) yOffset, canvas.size().x, preferredSize.y);
    ...
    public Vector2 getPreferredContentSize(Canvas canvas, Vector2 areaHint) {
        ...
        for (T item : list.get()) {
            Vector2 preferredSize = canvas.getCurrentStyle().getMargin().grow(itemRenderer.getPreferredSize(item, canvas));
