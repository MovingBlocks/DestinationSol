...
import org.destinationsol.nui.widgets.UIScrollbar;
...
public class ScrollableArea extends CoreLayout {
    ...
    public void onDraw(Canvas canvas) {
        ...
        Vector2 contentSize = canvas.calculateRestrictedSize(content, canvasSize);

        int horizontalScrollbarHeight = (int) canvas.calculateRestrictedSize(horizontalBar, canvasSize).y;
        int verticalScrollbarWidth = (int) canvas.calculateRestrictedSize(verticalBar, canvasSize).x;
     ...
     * @param contentSize   The size of the content
     */
    private void drawWithJustVertical(Canvas canvas, Vector2 availableSize, Vector2 contentSize) {
        boolean atBottom = verticalBar.getRange() == verticalBar.getValue();

        Rectangle contentRegion = new Rectangle(0,0, availableSize.x,availableSize.y);
        verticalBar.setRange((int) (contentSize.y - contentRegion.height));
        ...
        int scrollbarWidth = (int) canvas.calculateRestrictedSize(verticalBar, availableSize).x;
        canvas.drawWidget(verticalBar,
                new Rectangle(
                        availableSize.x, 0,
                        scrollbarWidth, availableSize.y));

        try (SubRegion ignored = canvas.subRegion(contentRegion, true)) {
            canvas.drawWidget(content, new Rectangle(0, -verticalBar.getValue(), availableSize.x, contentSize.y));
        }
    }
     ...
     */
    private void drawWithJustHorizontal(Canvas canvas, Vector2 availableSize, Vector2 contentSize) {
        Rectangle contentRegion = new Rectangle(0,0, availableSize.x,availableSize.y);

        canvas.addInteractionRegion(scrollListener);
        horizontalBar.setRange((int) (contentSize.x - contentRegion.width));
        int scrollbarHeight = (int) canvas.calculateRestrictedSize(verticalBar, availableSize).y;
        canvas.drawWidget(horizontalBar,
                new Rectangle(
                        0, availableSize.y,
                        availableSize.x, scrollbarHeight));

        try (SubRegion ignored = canvas.subRegion(contentRegion, true)) {
            canvas.drawWidget(content, new Rectangle(-horizontalBar.getValue(), 0, contentSize.x, availableSize.y));
        }
    }
    ...
    private void drawWithBoth(Canvas canvas,
        ...
        canvas.addInteractionRegion(scrollListener);
        Vector2 verticalWidgitSize = canvas.calculateRestrictedSize(verticalBar, availableSize);
        canvas.drawWidget(verticalBar,
                new Rectangle(availableSize.x, 0,verticalWidgitSize.x,verticalWidgitSize.y));
        Vector2 horizantalWidgitSize = canvas.calculateRestrictedSize(horizontalBar, availableSize);
        canvas.drawWidget(horizontalBar,
                new Rectangle(
                        0, availableSize.y,
                        horizantalWidgitSize.x,horizantalWidgitSize.y));

        try (SubRegion ignored = canvas.subRegion(contentRegion, true)) {
            canvas.drawWidget(content, new Rectangle(-horizontalBar.getValue(), -verticalBar.getValue(), contentSize.x, contentSize.y));
        }
    }
     ...
     */
    private void drawWithNeither(Canvas canvas, Vector2 availableSize) {
        canvas.drawWidget(content, new Rectangle(0,0, availableSize.x,availableSize.y));
    }


    @Override
    public Vector2 getPreferredContentSize(Canvas canvas, Vector2 sizeHint) {
        Vector2 pf = canvas.calculatePreferredSize(content);
        return new Vector2(preferredWidth == null ? pf.x : preferredWidth, preferredHeight == null ? pf.y : preferredHeight);
    }
