 ...
 */
package org.destinationsol.nui.layouts.miglayout;
...
public class MigLayout extends CoreLayout<MigLayout.CCHint> implements ContainerWrapper {
    ...
    public void onDraw(Canvas canvas) {
        ...
        for (ComponentWrapper wrapper : wrappers.values()) {
            ...
            Rectangle region = Rectangle.createFromMinAndSize(wrapper.getX(), wrapper.getY(), wrapper.getWidth(), wrapper.getHeight());
        ...
        for (Rectangle region : debugRects) {
    ...
    public Vector2 getPreferredContentSize(Canvas canvas, Vector2 sizeHint) {
        int[] bounds = {0, 0, canvas.size().x, canvas.size().y};
        layoutContainer(canvas, bounds);
        return new Vector2(grid.getWidth()[1], grid.getHeight()[1]);
    }

    @Override
    public Vector2 getMaxContentSize(Canvas canvas) {
        return getPreferredContentSize(canvas, Vector2.zero());
    }
    ...
    public void paintDebugCell(int x, int y, int width, int height) {
        debugRects.add(Rectangle.createFromMinAndSize(x, y, width, height));
    }
