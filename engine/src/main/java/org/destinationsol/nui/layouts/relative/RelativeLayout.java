...
public class RelativeLayout extends CoreLayout<RelativeLayoutHint> {
    ...
    private Map<WidgetInfo, Rectangle> cachedRegions = Maps.newHashMap();
    ...
    public void onDraw(Canvas canvas) {
        for (WidgetInfo element : contents) {
            Rectangle drawRegion = getRegion(element, canvas);
            canvas.drawWidget(element.widget, drawRegion);
        }
        cachedRegions.clear();
    }

    private Rectangle getRegion(WidgetInfo element, Canvas canvas) {
        Rectangle cachedRegion = cachedRegions.get(element);
        ...
        if (element.layoutHint.getPositionCenterHorizontal() != null) {
            ...
            Rectangle targetRegion = getTargetRegion(info.getWidget(), canvas);
        ...
        if (width == 0 && element.layoutHint.isUsingContentWidth()) {
            width = canvas.calculateRestrictedSize(element.widget, new Vector2(right - left, bottom - top)).x;
        }
        if (width == 0) {
            width = right - left;
        } else {
        ...
        if (height == 0 && element.layoutHint.isUsingContentHeight()) {
            height = canvas.calculateRestrictedSize(element.widget, new Vector2(width, bottom - top)).y;
        }
        if (height == 0) {
            height = bottom - top;
        } else {
        ...
        Rectangle region = Rectangle.createFromMinAndSize(left, top, width, height);
        cachedRegions.put(element, region);
        return region;
    }

    private Rectangle getTargetRegion(String id, Canvas canvas) {
        if (id != null && !id.isEmpty()) {
            ...
            if (target != null) {
                try {
                    Rectangle region = getRegion(target, canvas);
                    loopDetectionId = "";
                    return region;
