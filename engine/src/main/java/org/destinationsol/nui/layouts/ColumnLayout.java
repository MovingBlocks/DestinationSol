...
public class ColumnLayout extends CoreLayout<LayoutHint> {
        ...
        if (!widgetList.isEmpty()) {
            ...
            for (int rowIndex = 0; rowIndex < rows.size(); ++rowIndex) {
                ...
                for (int i = 0; i < row.size(); ++i) {
                    ...
                    if (widget != null) {
                        Rectangle drawRegion = Rectangle.createFromMinAndSize(cellOffsetX, rowOffsetY, minWidths[i], rowHeight);
    ...
    private RowInfo calculateRowSize(List<UIWidget> row, Canvas canvas, Vector2 areaHint) {
        ...
        for (int i = 0; i < columns && i < row.size(); ++i) {
            UIWidget widget = row.get(i);
            Vector2 cellSize = new Vector2(availableWidth, areaHint.y);
            if (!autoSizeColumns) {
            ...
            if (widget != null) {
                ...
                rowInfo.widgetSizes.add(new Vector2(0, 0));
            }
        }
        return rowInfo;
    }
    ...
    public Vector2 getMaxContentSize(Canvas canvas) {
        ...
        while (rows.hasNext()) {
            ...
            int rowHeight = 0;
            for (int i = 0; i < row.size(); ++i) {
                Vector2 maxSize = canvas.calculateMaximumSize(row.get(i));
