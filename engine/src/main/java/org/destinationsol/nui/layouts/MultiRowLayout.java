...
public class MultiRowLayout extends CoreLayout<LayoutHint> {
    ...
    public void onDraw(Canvas canvas) {
        if (!widgetList.isEmpty()) {
            Vector2 availableSize = canvas.size();
            ...
            for (int columnIndex = 0; columnIndex < columns.size(); ++columnIndex) {
                ...
                for (int i = 0; i < column.size(); ++i) {
                    ...
                    if (widget != null) {
                        Rectangle drawRegion = Rectangle.createFromMinAndSize(columnOffsetX, cellOffsetY, columnWidth, minHeights[i]);
    ...
    private ColumnInfo calculateColumnSize(List<UIWidget> column, Canvas canvas, Vector2 areaHint) {
        ...
        for (int i = 0; i < rows && i < column.size(); ++i) {
            UIWidget widget = column.get(i);
            Vector2 cellSize = new Vector2(areaHint.x, availableHeight);
            if (!autoSizeRows) {
                cellSize.y = (int) (cellSize.y * rowHeights[i]);
            }
            if (widget != null) {
                Vector2 contentSize = canvas.calculateRestrictedSize(widget, cellSize);
                columnInfo.widgetSizes.add(contentSize);
                columnInfo.width = Math.max(columnInfo.width, contentSize.x);
            } else {
                columnInfo.widgetSizes.add(new Vector2(0, 0));
            }
        }
        return columnInfo;
    }
    ...
    public Vector2 getPreferredContentSize(Canvas canvas, Vector2 areaHint) {
        Vector2 availableSize = new Vector2(areaHint);
        int numColumns = TeraMath.ceilToInt((float) widgetList.size() / rows);
        if (numColumns > 0) {
            availableSize.x -= horizontalSpacing * (numColumns - 1);
        }
    ...
    public Vector2 getMaxContentSize(Canvas canvas) {
        ...
        while (columns.hasNext()) {
            ...
            int columnWidth = 0;
            for (int i = 0; i < column.size(); ++i) {
                Vector2 maxSize = canvas.calculateMaximumSize(column.get(i));
    ...
    private static class ColumnInfo {
        private int width;
        private List<Vector2> widgetSizes = Lists.newArrayList();
