...
public class RowLayout extends CoreLayout<RowLayoutHint> {
    ...
    public void onDraw(Canvas canvas) {
        ...
        if (!contents.isEmpty()) {
            ...
            for (int i = 0; i < contents.size(); ++i) {
                ...
                Rectangle region = Rectangle.createFromMinAndSize(xOffset, 0, itemWidth, canvas.size().y);
                    ...
                    if (results.get(i) == 0) {
                        ...
                        if (hint != null) {
                            if (hint.isUseContentWidth()) {
                                Vector2 contentSize = contents.get(i).getPreferredContentSize(canvas, new Vector2(remainingWidthPerElement, canvas.size().y));
     ...
     * @param areaHint A {@link Vector2} representing the space available for widgets to be drawn in this layout
     * @return A {@link Vector2} representing the preferred content size of this {@code RowLayout}
     */
    @Override
    public Vector2 getPreferredContentSize(Canvas canvas, Vector2 areaHint) {
        TIntList widths = calcWidths(canvas);

        Vector2 result = new Vector2(areaHint.x, 0);
        for (int i = 0; i < contents.size(); ++i) {
            Vector2 widgetSize = canvas.calculateRestrictedSize(contents.get(i), new Vector2(TeraMath.floorToInt(widths.get(i)), areaHint.y));
     ...
     *
     * @param canvas The {@code Canvas} on which this {@code RowLayout} is drawn
     * @return A {@code Vector2} representing the maximum content size of this {@code RowLayout}
     */
    @Override
