...
public class ZoomableLayout extends CoreLayout {
    ...
    private InteractionListener dragListener = new BaseInteractionListener() {
        ...
        public void onMouseOver(NUIMouseOverEvent event) {
            last = new Vector2(event.getRelativeMousePosition());
        }
    ...
    protected void drawWidget(Canvas canvas, PositionalWidget widget) {
        ...
        Vector2 screenStart = worldToScreen(widget.getPosition());
        Vector2f worldEnd = new Vector2f(widget.getPosition());
        worldEnd.add(widget.getSize());
        Vector2 screenEnd = worldToScreen(worldEnd);
        canvas.drawWidget(widget, Rectangle.createFromMinAndMax(screenStart, screenEnd));
    }
    ...
    public Vector2f screenToWorld(Vector2 screenPos) {
        Vector2f world = new Vector2f(screenPos.x / pixelSize.x, screenPos.y / pixelSize.y);
        world.add(windowPosition);
        return world;
    }

    public Vector2 worldToScreen(Vector2f world) {
        return new Vector2(TeraMath.ceilToInt((world.x - windowPosition.x) * pixelSize.x), TeraMath.ceilToInt((world.y - windowPosition.y) * pixelSize.y));
    }
    ...
    public void zoom(float zoomX, float zoomY, Vector2 mousePos) {
