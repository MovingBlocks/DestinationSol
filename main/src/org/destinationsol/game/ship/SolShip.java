...
public class SolShip implements SolObject {
    ...
    public void update(SolGame game) {
        ...
        if (onFire) {
            game.getOggSoundManager().play(game, game.getSpecialSounds().burning, null, this);
        }
    ...
    private void updateAbility(SolGame game) {
        ...
        if (myAbilityAwait > 0) {
            ...
            if (myAbilityAwait <= 0) {
                game.getOggSoundManager().play(game, sounds.abilityRecharged, null, this);
            }
        }
    ...
    private void playHitSound(SolGame game, Vector2 pos, DmgType dmgType) {
        if (myArmor != null) {
            PlayableSound sound = myArmor.getHitSound(dmgType);
            game.getOggSoundManager().play(game, sound, pos, this);
        } else {
    ...
    public void disableControls(float duration, SolGame game) {
        if (myControlEnableAwait <= 0)
            game.getOggSoundManager().play(game, game.getSpecialSounds().controlDisabled, null, this);
