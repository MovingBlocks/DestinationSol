...
public class MainScreen implements SolUiScreen {
    ...
    private void drawHeight(UiDrawer drawer, Planet np, Vector2 camPos, float camAngle) {
        ...
        if (Constants.ATM_HEIGHT < toPlanet) {
            return;
        }
        float perc = toPlanet / Constants.ATM_HEIGHT;
        float sz = .08f;
        float maxY = 1 - sz / 2;
        float y = 1 - perc;
    ...
    private static class EnemyWarn extends WarnDrawer {
        ...
        protected boolean shouldWarn(SolGame game) {
            ...
            float heroCap = HardnessCalculator.getShipDmgCap(h);
            List<SolObject> objs = game.getObjMan().getObjs();
            ...
            for (int i = 0, sz = objs.size(); i < sz; i++) {
                ...
                dps += HardnessCalculator.getShipDps(ship);
                if (HardnessCalculator.isDangerous(heroCap, dps)) {
                    return true;
                }
            }
            return false;
