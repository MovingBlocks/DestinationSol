...
public class ShipKbControl implements ShipUiControl {
  ...
  public ShipKbControl(SolCmp cmp, float r, List<SolUiControl> controls) {
    ...
    leftCtrl = new SolUiControl(showButtons ? MainScreen.btn(colN1, rowN0) : null, false, Input.Keys.LEFT);
    leftCtrl.setDisplayName("Left");
    controls.add(leftCtrl);
    rightCtrl = new SolUiControl(showButtons ? MainScreen.btn(colN0, rowN0) : null, false, Input.Keys.RIGHT);
    ...
    shootCtrl = new SolUiControl(showButtons ? MainScreen.btn(col0, rowN1) : null, false, Input.Keys.SPACE);
    shootCtrl.setDisplayName("Primary");
    controls.add(shootCtrl);
    shoot2Ctrl = new SolUiControl(showButtons ? MainScreen.btn(col1, rowN0) : null, false, Input.Keys.CONTROL_LEFT);
