...
public class GunConfigs {
  ...
  public GunConfigs(TexMan texMan) {
    ...
    ProjectileFactory.BulletFactory weakBolterFac = new ProjectileFactory.BulletFactory(bolterTex, .12f, 5f, true);
    weakBolter = new GunConfig(1, 30, 6, 2, .8f, 2.5f, weakBolterFac, .12f, "bolter", "Weak Bolter", true, texMan, 15, "", 12, 2); //2.5

    ProjectileFactory.BulletFactory bolterFac = new ProjectileFactory.BulletFactory(bolterTex, .15f, 5.5f, true);
    ...
    ProjectileFactory.BulletFactory slowGunFac = new ProjectileFactory.BulletFactory(bulletTex, BULLET_W, 8f, false);
    slowGun = new GunConfig(1, 10, 6, 1, .2f, 1, slowGunFac, .24f, "slowGun", "Slow Gun", true, texMan, 50, "", 0, 2); //10

    ProjectileFactory.BulletFactory miniGunFac = new ProjectileFactory.BulletFactory(bulletTex, BULLET_W, 8f, false);
