...
public class SystemsBuilder {
  ...
  public List<SolSystem> build(List<SolSystem> systems, List<Planet> planets, PlanetConfigs planetConfigs) {
    for (int i = 0; i < SYS_COUNT; i++) {
      ...
      SolSystem s = createSystem(sysPos, planets, planetConfigs);
      SolMath.free(sysPos);
      systems.add(s);
    }
    return systems;
  ...
  private SolSystem createSystem(Vector2 sysPos, List<Planet> planets, PlanetConfigs planetConfigs) {
    ...
    for (int j = 0; j < PLANET_COUNT; j++) {
      ...
      Planet p = createPlanet(planetDist, s, groundHeight, planetConfig);
      planets.add(p);
      s.getPlanets().add(p);
      planetDist += reserved;
    }
